...
import os
from gtalkbot.PyGtalkRobot import GtalkRobot

BOT_GTALK_USER = 'domobot.automatic@gmail.com'
#BOT_GTALK_PASS = 'Domobot.automatic321'
#BOT_GTALK_USER = 'rigofran.olaya.vasquez@gmail.com'
BOT_GTALK_PASS = '1039452505'
BOT_ADMIN = '1ysieqrd8o0440r3jcgks5xvby@public.talk.google.com'

GPIO.setmode(GPIO.BOARD) # or GPIO.setmode(GPIO.BCM)
GPIO.setup(int(11), GPIO.OUT)
GPIO.setup(int(12), GPIO.OUT)
GPIO.setup(int(7), GPIO.IN)
#BTN_BUS = Button(7)
#LED_One = LED(11)
#LED_Two = LED(12)
#GPIO.output(int(11), True)
...
class RaspiBot(GtalkRobot):
    ...
    #This method turns on the specified GPIO pin
    def command_003_setmode(self, user, message, args):
        '''(mode|setmode)( +(.*))?$(?i)'''
	jid = user.getStripped()
	mode = args[1]
	print "Changing Mode!"
	...
	if "audio" in args[1]:
	    FLAG_Mode = 1
        self.replyMessage(user, "MODE: " + mode + " at: " + time.strftime("%Y-%m-%d %a %H:%M:%S", time.localtime()))
	
    #This method turns on the specified GPIO pin
    def command_003_next(self, user, message, args):
        '''(step|next|continue)( +(.*))?$(?i)'''
        print "Next stop!"
	global nextStop
	if nextStop > 2:
	    nextStop = 0;
	...
	audio = AUDIOS_BUS_PATH + AUDIOS_BUS_STOP[nextStop]
	audio = "/home/pi/welcome.wav"
	print "Playing Audio!"
	FLAG_Gtalk = True
	#Start Audio 
	GPIO.output(int(12), True)
        playAudio(audio)
        self.replyMessage(user, "The Next Stop audio file is: "+ audio +" at: "+time.strftime("%Y-%m-%d %a %H:%M:%S", time.localtime()))
	GPIO.output(int(12), False)
	FLAG_Gtalk = False
